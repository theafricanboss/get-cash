...
function display_venmo_shortcode($attr) {
    ...
    return '<p style="padding: 10px; max-width: 150px; text-align: center; border-radius: 10px;' .
        $display . $shadow . '">' . $button_text .
        '<a href="https://venmo.com/code?user_id='. esc_attr( wp_kses_post( $receiver_venmo ) ). '" target="_blank">' .
        '<img style="float: none!important; max-height:150px!important; max-width:100px!important;" alt="Square Cash app link" src="https://chart.googleapis.com/chart?cht=qr&chld=L|0&chs=150x150&chl=https://venmo.com/code?user_id='. esc_attr( wp_kses_post( $receiver_venmo ) ). '">' .
        '</a></p>';
